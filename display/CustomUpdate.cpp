...
CustomUpdateClass::CustomUpdateClass(const char* partition)
: _error(0)
, _buffer(0)
, _bufferLen(0)
, _size(0)
...
CustomUpdateClass& CustomUpdateClass::onProgress(THandlerFunction_Progress fn) {
    _progress_callback = fn;
    return *this;
}

void CustomUpdateClass::_reset() {
    if (_buffer)
        delete[] _buffer;
    _buffer = 0;
    _bufferLen = 0;
...
bool CustomUpdateClass::canRollBack(){
...
bool CustomUpdateClass::rollBack(){
    if(_buffer){ //Update is running
        return false;
    }
    const esp_partition_t* partition = esp_ota_get_next_update_partition(NULL);
...
bool CustomUpdateClass::begin(size_t size, int command, int ledPin, uint8_t ledOn) {
    if(_size > 0){
    ...
    if (command == U_FLASH) {
        ...
        _partition = esp_partition_find_first(ESP_PARTITION_TYPE_DATA, ESP_PARTITION_SUBTYPE_DATA_SPIFFS, _partitionName);
        if(!_partition){
            _error = UPDATE_ERROR_NO_PARTITION;
            return false;
        }
...
void CustomUpdateClass::_abort(uint8_t err){
    _reset();
    _error = err;
}

void CustomUpdateClass::abort(){
    _abort(UPDATE_ERROR_ABORT);
}

bool CustomUpdateClass::_writeBuffer(){
...
bool CustomUpdateClass::_verifyHeader(uint8_t data) {
    if(_command == U_FLASH) {
...
bool CustomUpdateClass::_verifyEnd() {
    if(_command == U_FLASH) {
...
bool CustomUpdateClass::setMD5(const char * expected_md5){
...
bool CustomUpdateClass::end(bool evenIfRemaining){
    if(hasError() || _size == 0){
        return false;
    }
...
size_t CustomUpdateClass::write(uint8_t *data, size_t len) {
...
size_t CustomUpdateClass::writeStream(Stream &data) {
    size_t written = 0;
    size_t toRead = 0;
    if(hasError() || !isRunning())
        return 0;
...
void CustomUpdateClass::printError(Stream &out){
    out.println(_err2str(_error));
}

const char * CustomUpdateClass::errorString(){
